...
export const OutputConsole: React.FC<OutputConsoleProps> = ({
  ...
  const getStatusIcon = () => {
    if (isRunning) return <Terminal className="w-3 h-3 sm:w-4 sm:h-4 animate-spin" />;
    if (error) return <X className="w-3 h-3 sm:w-4 sm:h-4 text-red-500" />;
    if (success) return <Check className="w-3 h-3 sm:w-4 sm:h-4 text-green-500" />;
  ...
  return (
    <div className={`bg-gray-900 rounded-lg border-2 transition-all duration-300 ${
      ...
      <div className="flex items-center justify-between p-2 sm:p-3 border-b border-gray-700 bg-gray-800 rounded-t-lg">
        ...
        {testResults && (
          <div className="flex items-center space-x-2">
            <span className="text-gray-400 font-mono text-xs sm:text-sm">Tests:</span>
      ...
      <div 
        ref={consoleRef}
        className={`p-3 sm:p-4 font-mono text-xs sm:text-sm min-h-[150px] sm:min-h-[200px] max-h-[250px] sm:max-h-[300px] overflow-y-auto ${
